...
class OTTrainer(Trainer):
    ...
    def ot_init_demos(self):
        with open(f"/home/burson/data/tdmpc_ot/expert_demos/{self.cfg.task}.pkl", "rb") as f:
            ...
            input_tensor = torch.as_tensor(rgb_obs).float().to(self.device)
        ...
        while self._step <= self.cfg.steps:
            ...
            if done.any():
                ...
                if self.cfg.ot_reward_shaping and global_episode > 0:
                    ...
                    print(f'ot_rewards_reshaped:{ot_rewards_reshaped}, min:{ot_rewards_reshaped.min().item():.3f}, max:{ot_rewards_reshaped.max().item():.3f}, mean:{ot_rewards_reshaped.mean().item():.3f}')
                    sys.exit()
