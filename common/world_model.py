...
class WorldModel(nn.Module):
    ...
    def __repr__(self):
        ...
        modules = ["Encoder", "Dynamics", "Reward", "OT-Reward", "Policy prior", "Q-functions"]
        for i, m in enumerate(
            [self._encoder, self._dynamics, self._reward, self._ot_reward, self._pi, self._Qs]
        ):
            repr += f"{modules[i]}: {m}\n"
    ...
    def pi_prior(self, z, task):
        ...
        pi = mu + eps * log_std.exp()
        pi = math.pi_squash(pi)

        return pi, mu, log_std.exp()
